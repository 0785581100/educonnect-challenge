@foreach ($records as $record)
    <div class="bg-white rounded-lg shadow p-4 mb-6">
        <div class="flex items-center justify-between mb-2">
            <div class="font-bold text-lg">{{ $record->title }}</div>
            <span class="text-xs px-2 py-1 rounded {{ $record->status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-200 text-gray-600' }}">
                {{ ucfirst($record->status) }}
            </span>
        </div>
        <div class="text-sm text-gray-600 mb-1">Instructor: {{ optional($record->instructor)->name }}</div>
        <div class="text-sm text-gray-600 mb-1">Price: ${{ $record->price }}</div>
        <div class="text-sm text-gray-600 mb-1">Enrollments: {{ $record->enrollments()->count() }}</div>
        <div class="flex gap-4 mt-4 items-center">
            <a href="{{ url('/admin/courses/' . $record->id . '/edit') }}"
            ...
            <form action="{{ url('/admin/courses/' . $record->id) }}" method="POST" onsubmit="return confirm('Are you sure?');" style="display:inline;">
                @csrf
                @method('DELETE')
                <button type="submit"
